...
import { approveProduct } from "@/services/approveProductService";
...
export const ApproveProduct: React.FC<ApproveProductProps> = ({
  product,
  onLoadData,
}) => {
  ...
  const onSubmited = async () => {
    try {
      const response: ResponseStatus = await approveProduct(product.id);
  ...
  return (
    <section className="approve-product" id="approve-product">
      <AcctionButton
