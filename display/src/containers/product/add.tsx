...
import { ProductFormValues } from "@/datasources/domain/interface/product";
...
export const AddProduct: React.FC<AddProductProps> = ({ onLoadData }) => {
  ...
  const onSubmit: SubmitHandler<ProductFormValues> = async (data) => {
    try {
      const response: ResponseStatus = await addProduct(
        data.product,
        data.description,
        data.price
      );
  ...
  return (
    <section className="add-product" id="add-product">
      <CardButton
        size="8rem"
